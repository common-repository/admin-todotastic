...
use Richdho\Utils\Assets as UtilsAssets;
...
class Assets {
    ...
    public function enqueue_scripts_and_styles($type, $hook_suffix = null) {
        ...
        if ($type === self::$TYPE_ADMIN) {
            ...
            $this->enqueueStyle('metabox', 'metabox.css', ['admin']);
            $this->enqueueStyle('dashboard', 'dashboard.css', ['admin']);
        } elseif ($type === self::$TYPE_FRONTEND) {
            // include frontend files
        }

        // Localize script with server-side variables
        wp_localize_script($handle, BDOTSTD_SLUG_CAMELCASE, $this->localizeScript($type));
    }
